"""Feature engineering for the IPL auction pricing model.

The project is intentionally built around pre-auction information. Seasonal
performance features are shifted before they are merged into auction rows, so
the model does not learn from the season it is trying to price.
"""
...
def build_match_season_lookup(matches_df: pd.DataFrame) -> pd.DataFrame:
    """Return the match-to-season mapping used to enrich ball-by-ball rows."""
...
def attach_season_to_deliveries(
    deliveries_df: pd.DataFrame, matches_df: pd.DataFrame
) -> pd.DataFrame:
    """Attach season labels to deliveries using the match metadata table."""
...
def _batting_base(deliveries: pd.DataFrame) -> pd.DataFrame:
    """Create row-level batting flags before aggregation."""
    ...
    dismissal_text = (
        out["player_dismissed"].astype(str)
        if "player_dismissed" in out.columns
        else pd.Series("", index=out.index)
    )
    out["batter_dismissed"] = (
        dismissal_text.map(canonicalize_player_name).eq(out["batter_name_clean"]).astype(int)
    )
...
def engineer_batting_features(deliveries_df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate ball-by-ball data into player-season batting features."""
    ...
    summary["batting_strike_rate"] = 100 * safe_divide(
        summary["runs_scored"], summary["balls_faced"]
    )
    summary["batting_average"] = safe_divide(summary["runs_scored"], summary["dismissals"])
    summary["boundary_ball_pct"] = 100 * safe_divide(
        summary["fours"] + summary["sixes"], summary["balls_faced"]
    )
    summary["dot_ball_pct_batting"] = 100 * safe_divide(
        summary["dot_balls_batting"], summary["balls_faced"]
    )
    ...
    if not phase_summary.empty:
        phase_pivot = phase_summary.pivot_table(
            index=["match_season", "batter_name_clean"],
            columns="phase",
...
def _bowling_base(deliveries: pd.DataFrame) -> pd.DataFrame:
    """Create row-level bowling flags before aggregation."""
    ...
    out["bowler_wicket"] = (
        out["is_wicket"].eq(1)
        & out["dismissal_kind"].astype(str).str.lower().isin(DISMISSALS_CREDITED_TO_BOWLER)
    ).astype(int)
    out["phase"] = np.select(
        [out["over"].between(1, 6), out["over"].between(7, 15), out["over"] >= 16],
        ["powerplay", "middle", "death"],
...
def engineer_bowling_features(deliveries_df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate ball-by-ball data into player-season bowling features."""
    ...
    summary["economy_rate"] = safe_divide(
        summary["runs_conceded"], summary["legal_balls_bowled"] / 6.0
    )
    summary["dot_ball_pct_bowling"] = 100 * safe_divide(
        summary["dot_balls_bowling"], summary["legal_balls_bowled"]
    )

    phase_summary = (
        bowling.groupby(["match_season", "bowler_name_clean", "phase"], dropna=False)
        .agg(
            phase_runs=("runs_conceded_bowler", "sum"),
            phase_balls=("is_legal_ball", "sum"),
            phase_wickets=("bowler_wicket", "sum"),
        )
        .reset_index()
    )
    if not phase_summary.empty:
...
def build_player_season_features(
    ...
    """Build historical player features available before each auction.

    Rolling, career, and peak-season metrics are shifted by one season. For
    example, a 2025 auction row can use 2024-and-earlier IPL evidence, but not
    2025 post-auction outcomes.
    """
    ...
    merged = batting.merge(
        bowling, on=["match_season", "player_name_clean"], how="outer", suffixes=("", "_bowl")
    )
    ...
    for feature in ["runs_scored", "wickets", "batting_strike_rate", "economy_rate"]:
        ...
        merged[f"recent_{feature}"] = grouped[feature].transform(
            lambda s: s.shift(1).rolling(2, min_periods=1).mean()
        )

    merged["prior_seasons_played"] = grouped["match_season"].transform(
        lambda s: s.shift(1).notna().cumsum()
    )
    merged["career_runs_before_auction"] = grouped["runs_scored"].transform(
        lambda s: s.shift(1).cumsum()
    )
    merged["career_wickets_before_auction"] = grouped["wickets"].transform(
        lambda s: s.shift(1).cumsum()
    )
    merged["career_matches_before_auction"] = grouped["matches_played"].transform(
        lambda s: s.shift(1).cumsum()
    )
    merged["three_year_runs_before_auction"] = grouped["runs_scored"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).sum()
    )
    merged["three_year_wickets_before_auction"] = grouped["wickets"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).sum()
    )
    merged["three_year_matches_before_auction"] = grouped["matches_played"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).sum()
    )
    merged["three_year_batting_sr_before_auction"] = grouped["batting_strike_rate"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).mean()
    )
    merged["three_year_economy_before_auction"] = grouped["economy_rate"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).mean()
    )
    merged["career_best_runs_season_before_auction"] = grouped["runs_scored"].transform(
        lambda s: s.shift(1).cummax()
    )
    merged["career_best_wickets_season_before_auction"] = grouped["wickets"].transform(
        lambda s: s.shift(1).cummax()
    )
    merged["career_best_sr_season_before_auction"] = grouped["batting_strike_rate"].transform(
        lambda s: s.shift(1).cummax()
    )
    merged["career_best_economy_season_before_auction"] = grouped["economy_rate"].transform(
        lambda s: s.shift(1).cummin()
    )
    merged["prior_500_run_seasons"] = grouped["runs_scored"].transform(
        lambda s: s.shift(1).ge(500).cumsum()
    )
    merged["prior_400_run_seasons"] = grouped["runs_scored"].transform(
        lambda s: s.shift(1).ge(400).cumsum()
    )
    merged["prior_20_wicket_seasons"] = grouped["wickets"].transform(
        lambda s: s.shift(1).ge(20).cumsum()
    )
    merged["prior_15_wicket_seasons"] = grouped["wickets"].transform(
        lambda s: s.shift(1).ge(15).cumsum()
    )
    merged["pedigree_batter_index"] = (
        ...
        + 0.20
        * merged["three_year_batting_sr_before_auction"]
        .fillna(merged["three_year_batting_sr_before_auction"].median())
        .rank(pct=True)
    ...
    merged["pedigree_bowler_index"] = (
        ...
        * (
            1
            - merged["three_year_economy_before_auction"]
            .fillna(merged["three_year_economy_before_auction"].median())
            .rank(pct=True)
        )
...
def build_pre_auction_dataset(
    auction_df: pd.DataFrame, player_features: pd.DataFrame
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Merge auction rows to the latest pre-auction player information."""
    ...
    coverage.append(
        {
            ...
            "rows_after": len(merged),
            "retention_rate": len(merged) / coverage[-1]["rows_after"]
            if coverage[-1]["rows_after"]
            else np.nan,
        }
    )
    ...
    coverage.append(
        {
            ...
            "rows_before": coverage[-1]["rows_after"],
            "rows_after": int(merged["player_name_clean_perf"].notna().sum())
            if "player_name_clean_perf" in merged.columns
            else len(merged),
            "retention_rate": (
...
def build_team_season_summary(matches_df: pd.DataFrame) -> pd.DataFrame:
    """Create a lightweight team-results table for post-season review."""
    ...
    for _, row in matches.iterrows():
        ...
        outcome_text = " ".join(
            [str(row.get("outcome", "")), str(row.get("result", "")), str(row.get("method", "")),]
        ).lower()
        no_result = winner in ("", "No Result") and any(
            token in outcome_text for token in ["tie", "no result", "abandoned"]
        )
...
def build_realized_season_review(
    ...
    """Compare selected auction calls with realized post-season outcomes."""
    ...
    base["player_name_join"] = (
        base.get(
            "matched_player_name_clean", base.get("player_name_clean", base["player_name_display"])
        )
    ...
    realized["batter_component"] = (
        ...
        + 0.35
        * _zscore(realized["batting_strike_rate"].fillna(realized["batting_strike_rate"].median()))
    ...
    realized["bowler_component"] = (
        ...
        - 0.25
        * _zscore(realized["bowling_strike_rate"].fillna(realized["bowling_strike_rate"].median()))
    )
    realized["all_rounder_component"] = (
        0.5 * realized["batter_component"] + 0.5 * realized["bowler_component"]
    )
    ...
    team_summary = build_team_season_summary(
        matches_df if matches_df is not None else pd.DataFrame()
    )
    team_delta = pd.DataFrame()
    if not team_summary.empty:
        prev_summary = team_summary.loc[
            team_summary["match_season"].eq(realized_season - 1),
            ["team", "finish_rank", "made_playoffs"],
        ].rename(columns={"finish_rank": "finish_rank_prev", "made_playoffs": "made_playoffs_prev"})
        curr_summary = team_summary.loc[
            team_summary["match_season"].eq(realized_season),
            ["team", "finish_rank", "made_playoffs"],
        ].rename(
        ...
        team_delta["finish_rank_delta"] = (
            team_delta["finish_rank_prev"] - team_delta["finish_rank_current"]
        )
        team_delta["playoff_delta"] = team_delta["made_playoffs_current"].fillna(0) - team_delta[
            "made_playoffs_prev"
        ].fillna(0)
    ...
    captain_adjustment = 0.6 * season_rows["finish_rank_delta"].fillna(0) + 0.8 * season_rows[
        "playoff_delta"
    ].fillna(0)
    season_rows["captaincy_outcome_adjustment"] = np.where(
        season_rows["captaincy_proxy"].fillna(0).eq(1), captain_adjustment, 0.0,
    )
    season_rows["realized_news_index"] = season_rows["realized_value_index"].fillna(
        0
    ) + season_rows["captaincy_outcome_adjustment"].fillna(0)
    season_rows["realized_percentile_within_role"] = (
        season_rows.groupby("role_bucket_realized")["realized_news_index"]
        .rank(pct=True)
        .fillna(0.5)
    )
...
def build_selected_player_followup(
    ...
    """Add realized-season evidence to a hand-selected player subset."""
    ...
    selected["player_name_join"] = (
        selected.get(
            ...
            selected.get("player_name_clean", selected["player_name_display"]),
        )
    ...
    selected = selected.drop(
        columns=[col for col in stale_review_cols if col in selected.columns], errors="ignore"
    )
    ...
    followup = selected.merge(realized_review, on="player_name_join", how="left")
    ...
    captain_adjustment = 0.6 * followup["finish_rank_delta"].fillna(0) + 0.8 * followup[
        "playoff_delta"
    ].fillna(0)
    followup["captaincy_outcome_adjustment"] = np.where(
        followup["captaincy_proxy"].fillna(0).eq(1), captain_adjustment, 0.0,
    )
    followup["realized_news_index"] = followup["realized_value_index_2025"].fillna(0) + followup[
        "captaincy_outcome_adjustment"
    ].fillna(0)
    ...
    followup["article_rating"] = (
        followup["realized_outcome_label"]
        .map({"impact": "hit", "solid": "mid", "below_expectation": "flop"})
        .fillna("mid")
    )
    ...
    followup["realized_statline"] = np.select(
        [all_rounder_mask, bowler_mask, batter_mask,],
        [
